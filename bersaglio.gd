...
func _ready():
	if GameManager.bersaglio2_morto:
		queue_free()
		return
	...
	var suono = AudioStreamPlayer.new()
	suono.name = "SuonoScoperto"
	suono.stream = load("res://AudioUccisione.wav")
	add_child(suono)
...
func scopri_player():
	...
	if distanza <= RAGGIO_ATTACCO:
		...
		$AnimatedSprite2D.play("idle")
		%Messaggio.text = "You've been spotted!"
...
func applica_il_codice():
	set_physics_process(false)
	GameManager.bersaglio2_morto = true
	await get_tree().create_timer(0.7).timeout
	$AnimatedSprite2D.play("hurt")
	await $AnimatedSprite2D.animation_finished
	$AnimatedSprite2D.play("death")
	await $AnimatedSprite2D.animation_finished
	get_tree().change_scene_to_file("res://vittoria.tscn")
